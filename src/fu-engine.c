...
static gboolean
fu_engine_plugins_open(FuEngine *self, GError **error)
{
	const gchar *fn;
	...
	while ((fn = g_dir_read_name(dir)) != NULL) {
		...
		/* open the plugin and call ->class_init() */
...
fu_engine_load(FuEngine *self, FuEngineLoadFlags flags, GError **error)
{
	...
	/* load plugins early, as we have to call ->class_init() *before* building quirk silo */
	if (!fu_engine_plugins_open(self, error)) {
